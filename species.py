...
def generate(parameters):
    ...
    h_left = '''<rect height="110.0" width="50.0" opacity="{opacity}" fill="%s"/>''' % colors[parameters['h_left_color']]
    ...
    h_right = '''<rect height="110.0" width="50.0" opacity="{opacity}" fill="%s" x="60.0"/>''' % colors[parameters['h_right_color']]
    ...
    ''' % (colors[parameters['a_outer_color']], colors[parameters['a_inner_color']])
    c_bottom = '''<g transform="translate(0,110.0) scale(1,-1)">
          ...
          </g>''' % colors[parameters['c_bottom_color']]
    c_top = '''<path opacity="{opacity}" d="M 99.49593469062211,22.671811123913976 A 55.0,55.0 0 1 0 33.95241121992003 105.81337428812076 Z" fill="%s"/>''' % colors[parameters['c_top_color']]
    ...
    '''.format(*c_parts)
    k_top = '''<polygon points="0,110.0 0,0 110.0,0 50.0,110.0" opacity="{opacity}" fill="%s"/>''' % colors[parameters['k_top_color']]
    k_bottom = '''<polygon points="0,0 0,110.0 110.0,110.0 50.0,0" opacity="{opacity}" fill="%s"/>''' % colors[parameters['k_bottom_color']]
